...
def section_track_b() -> str:
    """transformers GPU runs, one block per model."""
    files = sorted(RAW.glob("gpu_transformers*.jsonl"))
    parts = ["## 3. Track B -- best realistic GPU deployment (transformers, 2x A40)\n"]
    if not files:
    ...
    if not all_rows:
        parts.append("_No usable records found._\n")
        return "\n".join(parts)

    # Group by model so several checkpoints can be compared side by side.
    by_model = defaultdict(list)
    for r in all_rows:
        by_model[r.get("model", "unknown")].append(r)

    if len(by_model) > 1:
        parts.append("**Decode throughput by model** (higher is better)\n")
        head = []
        for model, rs in by_model.items():
            dec = [r["decode_tok_s"] for r in rs if r.get("decode_tok_s")]
            pre = [r["prefill_tok_s"] for r in rs if r.get("prefill_tok_s")]
            vram = [r["telemetry"].get("peak_vram_mib") for r in rs
                    if r["telemetry"].get("peak_vram_mib")]
            head.append([
                model,
                rs[0].get("precision"),
                fmt(statistics.fmean(dec) if dec else None),
                fmt(max(pre) if pre else None),
                fmt(max(vram) if vram else None, 0),
            ])
        parts.append(md_table(
            ["Model", "Precision", "Mean decode tok/s", "Peak prefill tok/s",
             "Peak VRAM (MiB)"], head))
        parts.append("")

    for model, model_rows in by_model.items():
        parts.append("\n### " + model + "\n")
        parts.append(_track_b_table(model_rows))
    return "\n".join(parts)


def _track_b_table(rows) -> str:
...
def main():
    ...
    doc = [
        "# Local LLM benchmark -- CPU vs GPU, and Uzbek quality\n",
        "_Generated by `analyze/make_report.py`. Each model is labelled by its "
        "Hugging Face id; run several and they appear side by side._\n",
        section_host(),
        section_track_a(),
        section_track_b(),
        section_uzbek(),
